...
const BecomeProvider = () => {
    ...
    useEffect(() => {
        ...
        handleFetch(
            ...
            '/api/get-provinces',
            {method: "POST", body: JSON.stringify({country: userCountry})},
    ...
    useEffect(() => {
        ...
        handleFetch(
            ...
            '/api/get-cities',
            {method: "POST", body: JSON.stringify({province: userProvince})},
            globalFetchFailureMsg,
            setExistingCities
        )
    }, [userProvince])
    ...
    if(application && application.status !== "rejected") {
        return (
            <ProtectedDashboardLayout>
                <div className="container border rounded p-4 w-50 my-5 text-center">
            ...
            <div className="container border rounded p-2 w-50 my-5">
                ...
                <form className="d-flex flex-column gap-2" onSubmit={handleProviderConversion}>
                    ...
                    <div className="d-flex row-gap-1 flex-column">
                        ...
                        {/* the schema enum is lowercase, so these values have to be too */}
                        <label htmlFor="product">
                            <input className="me-1" value="product" checked={userCategoryType === 'product'} onChange={handleCategoryType} type="radio" name="userCategoryType" id="product" />
                            Product
                        </label>
                        <label htmlFor="service">
                            <input className="me-1" value="service" checked={userCategoryType === 'service'} onChange={handleCategoryType} type="radio" name="userCategoryType" id="service" />
                            Service
                        </label>
                    </div>
